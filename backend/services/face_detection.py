...
class FaceDetection:
    ...
    @staticmethod
    def draw(label: str, rgb_img: cv2.UMat, faces: list, display_confidence=False, display_landmark=False):
        for i, face in enumerate(faces):
            x, y, w, h = face['box']
            cv2.rectangle(rgb_img, (int(x), int(y)), (int(x + w), int(y + h)), (255, 0, 0), 2)
            if display_confidence:
                confidence = str(face['confidence'])
                if y > 20:
                    cv2.putText(rgb_img, confidence, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9,
                                (36, 255, 12),
                                2)
                else:
                    cv2.putText(rgb_img, confidence, (x, y + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.9,
                                (36, 255, 12),
                                2)
            if display_landmark:
